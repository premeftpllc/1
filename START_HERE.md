...
| `config.yaml` | **Secret-free since 2026-09-24**: the 4 literals are now `${{ secrets.X }}` resolved from `~/.continue/.env` (generated by `scripts/sync-secrets.ps1` from SOPS). Pre-swap backup: `~/.continue-backups/2026-09-24/`. Still gitignored (Windows paths make it PC-specific). |
| Rules | Single copy in `~/.continue/rules`. Continue loads global AND workspace rule dirs without de-duplicating, so the identical workspace copy doubled every rule (~5,800 wasted tokens/request). Workspace copy moved to `~/.continue-backups/2026-09-24/workspace-rules`. |
...
### Owner decision 2026-09-24: Nemotron on the Mac
The owner plans to run `nvidia/nemotron-3-nano-4b` in Continue on the MacBook Neo too, so ignore
the Gemma/Qwen references above for the Mac's future state. It is not installed on the Mac yet. At
switch time the Mac updates its own config, `scripts/workspace.py` MODEL, the "Load local model"
task and its docs. Context length is the owner's call (the Mac has 8 GB RAM; the earlier decision
was 131,072).
