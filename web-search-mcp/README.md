...
### 2. Web Search Summaries Details
...
### 3. Single Web Page Content Details
...
The server uses MCP over standard input/output, so run the container interactively:

```bash
docker run --rm -i web-search-mcp
```

For an MCP client that supports Docker commands, configure it to launch the server with:

```json
{
  "command": "docker",
  "args": ["run", "--rm", "-i", "web-search-mcp"]
}
```

Configuration values can be passed with Docker's `-e` option, for example:

```bash
docker run --rm -i -e MAX_BROWSERS=1 web-search-mcp
```
